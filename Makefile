src/asm.so: src/asm.S
	cd src && \
	gcc -g3 -O0 asm.S -shared -o asm.so

src/fortran.so: src/fortran.f90
	cd src && \
	gfortran -fPIC -g3 -O0 fortran.f90 -shared -o fortran.so

src/librust.so: FORCE
	cd src/rust && \
	cargo build  && \
	cp target/debug/librust.so ../librust.so

src/c.so: src/c.c
	cd src && \
	gcc -fPIC -g3 -O3 c.c -shared -o c.so
